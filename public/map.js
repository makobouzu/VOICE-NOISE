    ...
    .then(response => {
        ...
        sounds.map(s => {
            var marker = new mapboxgl.Marker({ "color": "#222529" })
                ...
                .setHTML(`<p class="fw-bold">${s.name}</p><audio src ="${s.path}" onplay="gtag('event', 'Click', {'event_category': 'marker_${s.name}', 'event_label': 'maker_${s.name}', 'non_interaction': true});" controls>`))
                .addTo(map);
        });
    })
    .catch(err => {
